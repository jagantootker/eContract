...
class UpdateKontrakRequest extends FormRequest
{
    ...
    {
        $kontrakId = $this->route('id');

        return [
            'no_kontrak'                    => ['sometimes', 'required', 'string', 'max:50', "unique:kontrak,no_kontrak,{$kontrakId}", 'regex:' . self::NO_KONTRAK_PATTERN],
            'status_kontrak'                => ['sometimes', 'required', 'in:DRAF,DALAM_PELAKSANAAN,KONTRAK_SELESAI,EOT'],
            'tajuk_kontrak'                 => ['sometimes', 'required', 'string'],
            'syarikat_id'                   => ['sometimes', 'required', 'exists:syarikat,id'],
            'nilai_kontrak'                 => ['sometimes', 'required', 'numeric', 'min:0'],
            'kaedah_perolehan'              => ['sometimes', 'required', 'in:SEBUT HARGA,TENDER,RUNDINGAN TERUS,PEMBELIAN TERUS'],
            'kategori_perolehan'            => ['sometimes', 'required', 'in:PERKHIDMATAN,BEKALAN,KERJA'],
            ...
            'mula_tarikh'                   => ['sometimes', 'required', 'date'],
            'tamat_tarikh'                  => ['sometimes', 'required', 'date', 'after_or_equal:mula_tarikh'],
            'tarikh_sst'                    => ['nullable', 'date'],
            'status_draf_kompan'            => ['sometimes', 'boolean'],
            'tarikh_draf_hantar_sistem'     => ['nullable', 'date'],
            'jabatan_id'                    => ['nullable', 'exists:jabatan,id'],
            'bahagian_unit_id'              => ['nullable', 'exists:bahagian_unit,id'],
            'pegawai_bertanggungjawab_id'   => ['sometimes', 'required', 'exists:users,id'],
